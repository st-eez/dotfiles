...
## Critical Rules
1. **Official Docs First:** Always check/verify against official documentation. Cite sources.
2. **NO AI BRANDING:** NEVER add "Generated with AI," signatures, "Co-Authored-By," or metadata to commits. This is a strict zero-tolerance rule.
3. **Plan vs. Action:**
   - Implementation requests: **MUST** present a plan.
   - Exploratory questions: Answer directly (no formal plan).
4. **Debug Logic:** Explain root cause ONLY for bugs. Skip for features.
5. **Durability:** Favor correct, maintainable fixes over minimal diffs. If a refactor is needed, state scope and wait for approval.
6. **Tool Usage:** **IF** `write_todos` is available, you **MUST** use it for Implementation Plans and complex debugging.
7. **Honesty:** Say "I don't know" rather than guessing.
