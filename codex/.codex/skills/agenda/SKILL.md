...
```sh
SKILL_DIR="$HOME/.codex/skills/agenda"
...
When you want the rendered table to appear in your final assistant message instead of noisy shell output, use this pattern:

1. Render to a temp file, not stdout.
2. Inspect the temp file from disk before reasoning.
3. Include the table contents exactly once in your reply.
4. Do not print the table from shell output and then repeat it again in prose.
...
3. Inspect the temp file contents from disk.
4. Reason from the file contents, not from noisy shell output.
...
- Use the `YYYY-MM-DD` date-only portion of `last_planning_completed_at` when building Jira delta queries.
...
- If a line starting with `Jira:` contains a ticket key such as `NS-678` or `IT-574`, use that exact key in the Jira column. If `Jira:` is `none`, absent, or malformed, leave the Jira column blank. Do not infer Jira keys from the title.
...
- Use ID prefixes such as `4A83` when targeting reminders. Never use numeric indexes.
...
Inspect `OVERDUE_RAW`. If there are no overdue reminders, say so in one line and move to Phase 2.
...
Inspect `OVERDUE_TABLE` and include it once in your reply.
...
Inspect `JIRA_NEW_RAW` and `JIRA_CHANGED_RAW`.
...
If you render any of these tables, inspect the temp file and include each table once in your response.
...
Inspect `TODAY_TABLE` and `OPTIONAL_TABLE` if present, then include each table once in your reply.
...
Inspect the table files you actually need to cite, then include them once in your reply.
