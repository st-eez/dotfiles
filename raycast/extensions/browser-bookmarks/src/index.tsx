...
export default function Command() {
  ...
  const filteredBookmarks = useMemo(() => {
    if (query === "") return folderBookmarks;
  ...
  function mutateBookmarks() {
    if (hasArc) arc.mutate();
    if (hasBrave) brave.mutate();
    if (hasBraveBeta) braveBeta.mutate();
    if (hasBraveNightly) braveNightly.mutate();
    if (hasChatGPTAtlas) chatGPTAtlas.mutate();
    if (hasChrome) chrome.mutate();
    if (hasChromeBeta) chromeBeta.mutate();
    if (hasChromeDev) chromeDev.mutate();
    if (hasDia) dia.mutate();
    if (hasEdge) edge.mutate();
    if (hasEdgeCanary) edge.mutate();
    if (hasEdgeDev) edge.mutate();
    if (hasFirefox || hasFirefoxDev) firefox.mutate();
    if (hasGhostBrowser) ghostBrowser.mutate();
    if (hasHelium) helium.mutate();
    if (hasIsland) island.mutate();
    if (hasPrismaAccess) prismaAccess.mutate();
    if (hasSafari) safari.mutate();
    if (hasSidekick) sidekick.mutate();
    if (hasVivaldi) vivaldi.mutate();
    if (hasWhale) whale.mutate();
    if (hasZen) zen.mutate();
  }
              ...
              <ActionPanel>
                ...
                <ActionPanel.Section>
                  <Action.CreateQuicklink
                    ...
                    icon={Icon.Link}
                    quicklink={{ name: item.title, link: item.url }}
...
function SelectProfileSubmenu({
  ...
  return (
    <ActionPanel.Submenu title={`Select ${name} Profile`} icon={icon as any} shortcut={shortcut}>
