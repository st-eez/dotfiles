...
export async function safeExec(command: string, args: string[], timeoutMs: number = DEFAULT_TIMEOUT_MS): Promise<string> {
    console.log(`Executing: ${command} ${args.join(" ")}`);
    const basePath = process.env.PATH ?? "";
    const normalizedPath = basePath.includes("/opt/homebrew/bin")
        ? basePath
        : `/opt/homebrew/bin:${basePath}`;
    try {
