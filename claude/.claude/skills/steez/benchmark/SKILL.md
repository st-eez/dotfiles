...
echo '{"skill":"steez-benchmark","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}' >> "$STEEZ_HOME/analytics/skill-usage.jsonl" 2>/dev/null || true
```
...
At the end of each major workflow step, rate your /steez-benchmark experience 0-10. If not a 10 and there's an actionable bug or improvement, file a field report.
...
{one sentence}
**Date:** {YYYY-MM-DD} | **Skill:** /steez-benchmark
```
...
echo '{"skill":"steez-benchmark","duration_s":"'"$_TEL_DUR"'","outcome":"OUTCOME","browse":"USED_BROWSE","session":"'"$_SESSION_ID"'","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.steez/analytics/skill-usage.jsonl 2>/dev/null || echo "[steez] WARNING: telemetry write failed" >&2
```
...
# /steez-benchmark — Performance Regression Detection
...
## User-invocable
When the user types `/steez-benchmark`, run this skill.

## Arguments
- `/steez-benchmark <url>` — full performance audit with baseline comparison
- `/steez-benchmark <url> --baseline` — capture baseline (run before making changes)
- `/steez-benchmark <url> --quick` — single-pass timing check (no baseline needed)
- `/steez-benchmark <url> --pages /,/dashboard,/api/health` — specify pages
- `/steez-benchmark --diff` — benchmark only pages affected by current branch
- `/steez-benchmark --trend` — show performance trends from historical data
...
Same as /steez-canary — auto-discover from navigation or use `--pages`.
...
For each page, collect performance metrics:
