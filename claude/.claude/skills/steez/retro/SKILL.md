...
echo '{"skill":"steez-retro","ts":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","repo":"'"$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")"'"}' >> "$STEEZ_HOME/analytics/skill-usage.jsonl" 2>/dev/null || true
```
...
At the end of each major workflow step, rate the `/steez-retro` experience 0-10. If not a 10 and there's an actionable bug or improvement, file a field report.
...
{one sentence}
**Date:** {YYYY-MM-DD} | **Skill:** /steez-retro
```
...
echo '{"skill":"steez-retro","duration_s":"'"$_TEL_DUR"'","outcome":"OUTCOME","browse":"USED_BROWSE","session":"'"$_SESSION_ID"'","ts":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'"}' >> ~/.steez/analytics/skill-usage.jsonl 2>/dev/null || echo "[steez] WARNING: telemetry write failed" >&2
```
...
# /steez-retro — Weekly Engineering Retrospective

Generates a comprehensive engineering retrospective analyzing commit history, work patterns, and code quality metrics. Team-aware: identifies the user running the command, then analyzes every contributor with per-person praise and growth opportunities. Designed for a senior IC/CTO-level builder using Claude Code as a force multiplier.

## User-invocable
When the user types `/steez-retro`, run this skill.

## Arguments
- `/steez-retro` — default: last 7 days
- `/steez-retro 24h` — last 24 hours
- `/steez-retro 14d` — last 14 days
- `/steez-retro 30d` — last 30 days
- `/steez-retro compare` — compare current window vs prior same-length window
- `/steez-retro compare 14d` — compare with explicit window
- `/steez-retro global` — cross-project retro across all AI coding tools (7d default)
- `/steez-retro global 14d` — cross-project retro with explicit window
...
```
Usage: /steez-retro [window | compare | global]
  /steez-retro              — last 7 days (default)
  /steez-retro 24h          — last 24 hours
  /steez-retro 14d          — last 14 days
  /steez-retro 30d          — last 30 days
  /steez-retro compare      — compare this period vs prior period
  /steez-retro compare 14d  — compare with explicit window
  /steez-retro global       — cross-project retro across all AI tools (7d default)
  /steez-retro global 14d   — cross-project retro with explicit window
```
...
cat ~/.steez/greptile-history.md 2>/dev/null || true
...
**Greptile signal (if history exists):** Read `~/.steez/greptile-history.md` (fetched in Step 1, command 8). Filter entries within the retro time window by date. Count entries by type: `fix`, `fp`, `already-fixed`. Compute signal ratio: `(fix + already-fixed) / (fix + already-fixed + fp)`. If no entries exist in the window or the file doesn't exist, skip the Greptile metric row. Skip unparseable lines silently.
...
**Skill Usage (if analytics exist):** Read `~/.steez/analytics/skill-usage.jsonl` if it exists. Filter entries within the retro time window by `ts` field. Separate skill activations (no `event` field) from hook fires (`event: "hook_fire"`). Aggregate by skill name. Present as:
...
**Eureka Moments (if logged):** Read `~/.steez/analytics/eureka.jsonl` if it exists. Filter entries within the retro time window by `ts` field. For each eureka moment, show the skill that flagged it, the branch, and a one-line summary of the insight. Present as:
...
```
  EUREKA /steez-office-hours (branch: garrytan/auth-rethink): "Session tokens don't need server storage — browser crypto API makes client-side JWT validation viable"
  EUREKA /steez-plan-eng-review (branch: garrytan/cache-layer): "Redis isn't needed here — Bun's built-in LRU cache handles this workload"
```
...
```json
{
  ...
  "authors": {
    "Steve D": { "commits": 32, "insertions": 2400, "deletions": 300, "test_ratio": 0.41, "top_area": "browse/" },
...
**Note:** Only include the `greptile` field if `~/.steez/greptile-history.md` exists and has entries within the time window. Only include the `backlog` field if `TODOS.md` exists. Only include the `test_health` field if test files were found (command 10 returns > 0). If any has no data, omit the field entirely.
...
### Plan Completion
Check review JSONL logs for plan completion data from /steez-ship runs this period:
...
When the user runs `/steez-retro global` (or `/steez-retro global 14d`), follow this flow instead of the repo-scoped Steps 1-14. This mode works from any directory — it does NOT require being inside a git repo.
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t ~/.steez/retros/global-*.json 2>/dev/null | head -5
```
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
today=$(date +%Y-%m-%d)
existing=$(ls ~/.steez/retros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `~/.steez/retros/global-${today}-${next}.json`:
...
```json
{
  ...
  "tweetable": "Week of Mar 14: 5 projects, 182 commits, 15.3k LOC | CC: 48, Codex: 8, Gemini: 3 | Focus: my-project (58%) | Streak: 52d"
}
```
...
When the user runs `/steez-retro compare` (or `/steez-retro compare 14d`):
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `~/.steez/retros/` (not `.context/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
