 ...
 *
 * Flow:
 *   1. Read .steez/browse.json for port + token
...
export function resolveServerScript(
  ...
  // Dev mode: cli.ts runs directly from src/core/
  ...
  // Compiled binary: derive the source tree from dist/browse → ../src/core/server.ts
  if (execPath) {
    const adjacent = path.resolve(path.dirname(execPath), '..', 'src', 'core', 'server.ts');
...
function cleanupLegacyState(): void {
  // No legacy state on Windows — /tmp and `ps` don't exist, and browse
