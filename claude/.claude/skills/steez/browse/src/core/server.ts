...
const SESSIONS_DIR = path.join(process.env.HOME || '/tmp', '.steez', 'browse', 'sidebar-sessions');
...
function createWorktree(sessionId: string): string | null {
  try {
    ...
    const worktreeDir = path.join(process.env.HOME || '/tmp', '.steez', 'browse', 'worktrees', sessionId.slice(0, 8));
...
function spawnClaude(userMessage: string, extensionUrl?: string | null): void {
  ...
  const agentQueue = process.env.SIDEBAR_QUEUE_PATH || path.join(process.env.HOME || '/tmp', '.steez', 'browse', 'sidebar-agent-queue.jsonl');
  const steezDir = path.dirname(agentQueue);
  ...
  try {
    fs.mkdirSync(steezDir, { recursive: true });
...
async function start() {
  ...
  const server = Bun.serve({
    ...
    fetch: async (req) => {
      ...
      // Cookie picker routes — auth required on data routes (HTML page is public)
      ...
      if (url.pathname === '/health') {
        ...
        return new Response(JSON.stringify({
          ...
          // token removed — see state file (mode 0o600) for extension bootstrap
          chatEnabled: true,
          agent: {
            status: agentStatus,
      ...
      // ─── Sidebar endpoints (auth required — token from state file) ────
