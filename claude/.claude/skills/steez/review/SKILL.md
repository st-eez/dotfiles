...
preamble-tier: 4
version: 1.0.0
description: Pre-landing PR review. Analyzes diff against the base branch for SQL safety, LLM trust boundary violations, conditional side effects, and other structural issues. Use when asked to "review this PR", "code review", "pre-landing review", or "check my diff". Proactively suggest when the user is about to merge or land code changes. (steez)
allowed-tools:
  - Bash
  - Read
  - Edit
...
echo '{"skill":"steez-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> "$STEEZ_HOME/analytics/skill-usage.jsonl" 2>/dev/null || true
```
...
types (e.g., /steez-review, /steez-ship). If you would have auto-invoked a skill, instead briefly say:
...
At the end of each major workflow step, rate your /steez-review experience 0-10. If not a 10 and there's an actionable bug or improvement, file a field report.
...
{one sentence}
**Date:** {YYYY-MM-DD} | **Skill:** /steez-review
```
...
echo '{"skill":"steez-review","duration_s":"'"$_TEL_DUR"'","outcome":"OUTCOME","browse":"USED_BROWSE","session":"'"$_SESSION_ID"'","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.steez/analytics/skill-usage.jsonl 2>/dev/null || echo "[steez] WARNING: telemetry write failed" >&2
```
...
You are running the `/steez-review` workflow. Analyze the current branch's diff against the base branch for structural issues that tests don't catch.
...
1. Read `TODOS.md` (if it exists). Read PR description (`gh pr view --json body --jq .body 2>/dev/null || true`).
   ...
   **If no PR exists:** rely on commit messages and TODOS.md for stated intent — this is the common case since /steez-review runs before /steez-ship creates the PR.
...
Read `~/.claude/skills/steez-review/checklist.md`.
...
Read `~/.claude/skills/steez-review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps.
...
2. **Read `~/.claude/skills/steez-review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
Consider writing tests before running /steez-ship.
```

This is INFORMATIONAL — does not block /steez-review. But it makes low coverage visible early so the developer can address it before reaching the /steez-ship coverage gate.
...
2. If the doc file was NOT updated in this branch but the code it describes WAS changed, flag it as an INFORMATIONAL finding:
   "Documentation may be stale: [file] describes [feature/component] but code changed in this branch. Consider running `/steez-document-release`."

This is informational only — never critical. The fix action is `/steez-document-release`.
...
After all review passes complete, persist the final `/steez-review` outcome so `/steez-ship` can
recognize that Eng Review was run on this branch.

Run:

```bash
$STEEZ_BIN/steez-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"commit":"COMMIT"}'
```
...
- **Fix-first, not read-only.** AUTO-FIX items are applied directly. ASK items are only applied after user approval. Never commit, push, or create PRs — that's /steez-ship's job.
