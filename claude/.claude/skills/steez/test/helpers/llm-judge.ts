 ...
 *
 * Uses `claude -p` subprocess instead of the Anthropic SDK so it works
 * with OAuth auth (no separate ANTHROPIC_API_KEY required).
 *
 * Ported from gstack/test/helpers/llm-judge.ts.
 */
...
/**
 * Call claude via `claude -p` subprocess with a prompt, extract JSON response.
 * Uses --max-turns 1 and no tools to keep the judge fast and focused.
 * Retries once on failure.
 */
export async function callJudge<T>(prompt: string): Promise<T> {
  const model = process.env.EVALS_JUDGE_MODEL ?? 'claude-sonnet-4-6';

  const runOnce = async (): Promise<string> => {
    // Write prompt to temp file to avoid shell escaping issues
    const promptFile = path.join(os.tmpdir(), `.judge-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    fs.writeFileSync(promptFile, prompt);

    const proc = Bun.spawn(
      ['sh', '-c', `cat "${promptFile}" | claude -p --model "${model}" --max-turns 1 --output-format text --dangerously-skip-permissions`],
    ...
    if (exitCode !== 0) {
      throw new Error(`Judge subprocess failed (exit ${exitCode}): ${stderr.slice(0, 500)}`);
    }
