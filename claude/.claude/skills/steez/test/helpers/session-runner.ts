 ...
 *
 * Ported from gstack/test/helpers/session-runner.ts.
 */
...
const STEEZ_DEV_DIR = path.join(os.homedir(), '.steez-dev');
const HEARTBEAT_PATH = path.join(STEEZ_DEV_DIR, 'e2e-live.json');
const PROJECT_DIR = path.dirname(getProjectEvalDir());
...
export async function runSkillTest(options: {
  ...
  if (browseErrors.length > 0 || exitReason !== 'success') {
    try {
      const failureDir = runDir || path.join(workingDirectory, '.steez', 'test-transcripts');
