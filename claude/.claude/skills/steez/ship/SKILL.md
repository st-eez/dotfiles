...
{one sentence}
**Date:** {YYYY-MM-DD} | **Skill:** /steez-ship
```
...
echo '{"skill":"steez-ship","duration_s":"'"$_TEL_DUR"'","outcome":"OUTCOME","browse":"USED_BROWSE","session":"'"$_SESSION_ID"'","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.steez/analytics/skill-usage.jsonl 2>/dev/null || echo "[steez] WARNING: telemetry write failed" >&2
```
...
You are running the `/steez-ship` workflow. This is a **non-interactive, fully automated** workflow. Do NOT ask for confirmation at any step. The user said `/steez-ship` which means DO IT. Run straight through and output the PR URL at the end.
...
Check diff size: `git diff <base>...HEAD --stat | tail -1`. If the diff is >200 lines, add: "Note: This is a large diff. Consider running `/steez-plan-eng-review` or `/steez-autoplan` for architecture-level review before shipping."
...
[ -f .steez/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
```
...
If user picks H → write `.steez/no-test-bootstrap` and continue without tests.
...
If user picks C → write `.steez/no-test-bootstrap`. Tell user: "If you change your mind later, delete `.steez/no-test-bootstrap` and re-run." Continue without tests.
...
- Switch to /steez-investigate mindset: root cause first, then minimal fix.
...
- Create an issue assigned to that person (use the platform detected in Step 0):
  - **If GitHub:**
    ...
    gh issue create \
      ...
      --body "Found failing on branch <current-branch>. Failure is pre-existing.\n\n**Error:**\n```\n<first 10 lines>\n```\n\n**Last modified by:** <author>\n**Noticed by:** steez /steez-ship on <date>" \
...
`/steez-ship` is a pre-merge gate, so always use full tier (Sonnet structural + Opus persona judges).
...
**Tier reference (for context — /steez-ship always uses `full`):**
| Tier | When | Speed (cached) | Cost |
|------|------|----------------|------|
| `fast` (Haiku) | Dev iteration, smoke tests | ~5s (14x faster) | ~$0.07/run |
| `standard` (Sonnet) | Default dev, `bin/test-lane --eval` | ~17s (4x faster) | ~$0.37/run |
| `full` (Opus persona) | **`/steez-ship` and pre-merge** | ~72s (baseline) | ~$1.27/run |
...
Write to `~/.steez/projects/{slug}/{user}-{branch}-ship-test-plan-{datetime}.md`:

```markdown
# Test Plan
Generated by /steez-ship on {date}
Branch: {branch}
Repo: {owner/repo}
...
for PLAN_DIR in "$HOME/.claude/plans" "$HOME/.codex/plans" ".steez/plans"; do
  [ -d "$PLAN_DIR" ] || continue
...
- Review report sections (`## STEEZ REVIEW REPORT`)
...
Automatically verify the plan's testing/verification steps using the `/steez-qa-only` skill.
...
**If NO_SERVER:** Skip with "No dev server detected — skipping plan verification. Run /steez-qa separately after deploying."

### 3. Invoke /steez-qa-only inline

Read the `/steez-qa-only` skill from disk:

```bash
cat ${CLAUDE_SKILL_DIR}/../qa-only/SKILL.md
```

**If unreadable:** Skip with "Could not load /steez-qa-only — skipping plan verification."

Follow the /steez-qa-only workflow with these modifications:
- **Skip the preamble** (already handled by /steez-ship)
- **Use the plan's verification section as the primary test input** — treat each verification item as a test case
- **Use the detected dev server URL** as the base URL
- **Skip the fix loop** — this is report-only verification during /steez-ship
...
4. **Apply the design checklist** against the changed files. For each item:
   ...
   - **[LOW] intent-based detection**: present as "Possible — verify visually or run /steez-design-review"
...
7. **After all fixes (auto + user-approved):**
   - If ANY fixes were applied: commit fixed files by name (`git add <fixed-files> && git commit -m "fix: pre-landing review fixes"`), then **STOP** and tell the user to run `/steez-ship` again to re-test.
...
- Message: "Steez recommends maintaining a TODOS.md organized by skill/component, then priority (P0 at top through P4, then Completed at bottom). See TODOS-format.md for the full format. Would you like to create one?"
...
1. Read the `/steez-document-release` skill: `cat ${CLAUDE_SKILL_DIR}/../document-release/SKILL.md`
...
doc updates — the user runs `/steez-ship` and documentation stays current without a separate command.
...
```bash
echo '{"skill":"ship","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","coverage_pct":COVERAGE_PCT,"plan_items_total":PLAN_TOTAL,"plan_items_done":PLAN_DONE,"verification_result":"VERIFY_RESULT","version":"VERSION","branch":"BRANCH"}' >> ~/.steez/projects/$SLUG/$BRANCH-reviews.jsonl
```
...
- **The goal is: user says `/steez-ship`, next thing they see is the review + PR URL + auto-synced docs.**
