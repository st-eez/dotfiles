...
{one sentence}
**Date:** {YYYY-MM-DD} | **Skill:** /steez-canary
```
...
echo '{"skill":"steez-canary","duration_s":"'"$_TEL_DUR"'","outcome":"OUTCOME","browse":"USED_BROWSE","session":"'"$_SESSION_ID"'","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.steez/analytics/skill-usage.jsonl 2>/dev/null || echo "[steez] WARNING: telemetry write failed" >\&2
```
...
# /steez-canary — Post-Deploy Visual Monitor
...
## User-invocable
When the user types `/steez-canary`, run this skill.

## Arguments
- `/steez-canary <url>` — monitor a URL for 10 minutes after deploy
- `/steez-canary <url> --duration 5m` — custom monitoring duration (1m to 30m)
- `/steez-canary <url> --baseline` — capture baseline screenshots (run BEFORE deploying)
- `/steez-canary <url> --pages /,/dashboard,/settings` — specify pages to monitor
- `/steez-canary <url> --quick` — single-pass health check (no continuous monitoring)
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".steez/canary-reports/baselines/<page-name>.png"
$B console --errors
$B perf
$B text
```
...
Save the baseline manifest to `.steez/canary-reports/baseline.json`:
...
Then STOP and tell the user: "Baseline captured. Deploy your changes, then run `/steez-canary <url>` to monitor."
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".steez/canary-reports/screenshots/pre-<page-name>.png"
$B console --errors
$B perf
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".steez/canary-reports/screenshots/<page-name>-<check-number>.png"
$B console --errors
$B perf
```
...
Screenshots:   .steez/canary-reports/screenshots/

VERDICT: [DEPLOY IS HEALTHY / DEPLOY HAS ISSUES — details above]
```

Save report to `.steez/canary-reports/{date}-canary.md` and `.steez/canary-reports/{date}-canary.json`.
