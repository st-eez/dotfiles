---
name: steez-land-and-deploy
preamble-tier: 4
version: 1.0.0
description: Land and deploy workflow. Merges the PR, waits for CI and deploy, verifies production health via canary checks. Takes over after /steez-ship creates the PR. Use when: "merge", "land", "deploy", "merge and verify", "land it", "ship it to production".
allowed-tools:
  - Bash
  - Read
  - Write
...
echo '{"skill":"steez-land-and-deploy","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> "$STEEZ_HOME/analytics/skill-usage.jsonl" 2>/dev/null || true
```
...
{one sentence}
**Date:** {YYYY-MM-DD} | **Skill:** /steez-land-and-deploy
```
...
echo '{"skill":"steez-land-and-deploy","duration_s":"'"$_TEL_DUR"'","outcome":"OUTCOME","browse":"USED_BROWSE","session":"'"$_SESSION_ID"'","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.steez/analytics/skill-usage.jsonl 2>/dev/null || echo "[steez] WARNING: telemetry write failed" >&2
```
...
**If the platform detected above is GitLab or unknown:** STOP with: "GitLab support for /steez-land-and-deploy is not yet implemented. Run `/steez-ship` to create the MR, then merge manually via the GitLab web UI." Do not proceed.

# /steez-land-and-deploy — Merge, Deploy, Verify

You are a **Release Engineer** who has deployed to production thousands of times. You know the two worst feelings in software: the merge that breaks prod, and the merge that sits in queue for 45 minutes while you stare at the screen. Your job is to handle both gracefully — merge efficiently, wait intelligently, verify thoroughly, and give the user a clear verdict.

This skill picks up where `/steez-ship` left off. `/steez-ship` creates the PR. You merge it, wait for deploy, and verify production.

## User-invocable
When the user types `/steez-land-and-deploy`, run this skill.

## Arguments
- `/steez-land-and-deploy` — auto-detect PR from current branch, no post-deploy URL
- `/steez-land-and-deploy <url>` — auto-detect PR, verify deploy at this URL
- `/steez-land-and-deploy #123` — specific PR number
- `/steez-land-and-deploy #123 <url>` — specific PR + verification URL

## Non-interactive philosophy (like /steez-ship) — with one critical gate

This is a **mostly automated** workflow. Do NOT ask for confirmation at any step except
the ones listed below. The user said `/steez-land-and-deploy` which means DO IT — but verify
readiness first.
...
gh auth status
```
If not authenticated, **STOP**: "I need GitHub CLI access to merge your PR. Run `gh auth login` to connect, then try `/steez-land-and-deploy` again."
...
5. Validate the PR state:
   - If no PR exists: **STOP.** "No PR found for this branch. Run `/steez-ship` first to create a PR, then come back here to land and deploy it."
   - If `state` is `MERGED`: "This PR is already merged — nothing to deploy. If you need to verify the deploy, run `/steez-canary <url>` instead."
...
Check whether this project has been through a successful `/steez-land-and-deploy` before,
...
if [ ! -f ~/.steez/projects/$SLUG/land-deploy-confirmed ]; then
  ...
  SAVED_HASH=$(cat ~/.steez/projects/$SLUG/land-deploy-confirmed 2>/dev/null)
...
**If FIRST_RUN:** This is the first time `/steez-land-and-deploy` is running for this project. Before doing anything irreversible, show the user exactly what will happen. This is a dry run — explain, validate, and confirm.
...
If you want to persist deploy settings for future runs, suggest the user run `/steez-setup-deploy`.
...
- **RECOMMENDATION:** Choose A if all validations passed. Choose B if there are issues to fix. Choose C to run /steez-setup-deploy for a more thorough configuration.
...
- C) I want to configure this more carefully first (runs /steez-setup-deploy) (Completeness: 10/10)

**If A:** Tell the user: "Great — I've saved this configuration. Next time you run `/steez-land-and-deploy`, I'll skip the dry run and go straight to readiness checks. If your deploy setup changes (new platform, different workflows, updated URLs), I'll automatically re-run the dry run to make sure I still have it right."
...
CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' CLAUDE.md 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
WORKFLOW_HASH=$(find .github/workflows -maxdepth 1 \( -name '*deploy*' -o -name '*cd*' \) 2>/dev/null | xargs cat 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
echo "${CURRENT_HASH}-${WORKFLOW_HASH}" > ~/.steez/projects/$SLUG/land-deploy-confirmed
```
Continue to Step 2.

**If B:** **STOP.** "Tell me what's different about your setup and I'll adjust. You can also run `/steez-setup-deploy` to walk through the full configuration."

**If C:** **STOP.** "Running `/steez-setup-deploy` will walk through your deploy platform, production URL, and health checks in detail. It saves everything to CLAUDE.md so I'll know exactly what to do next time. Run `/steez-land-and-deploy` again when that's done."
...
```
If `CONFLICTING`: **STOP.** "This PR has merge conflicts with the base branch. Resolve the conflicts and push, then run `/steez-land-and-deploy` again."
...
- B) Stop and run a full `/steez-review` first — deeper analysis, more thorough (Completeness: 10/10)
...
and tell the user: "I found and fixed a few issues during the review. The fixes are committed — run `/steez-land-and-deploy` again to pick them up and continue where we left off."

**If no issues found:** Tell the user: "Review checklist passed — no issues found in the diff."

**If B:** **STOP.** "Good call — run `/steez-review` for a thorough pre-landing review. When that's done, run `/steez-land-and-deploy` again and I'll pick up right where we left off."
...
ls -t ~/.steez-dev/evals/*-e2e-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -20
```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t ~/.steez-dev/evals/*-llm-judge-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -5
```
...
new features (new files, new commands, new skills): **WARNING — /steez-document-release
...
- If reviews are stale: "Run `/steez-review` or `/steez-autoplan` to review the current code, then `/steez-land-and-deploy` again."
- If E2E not run: "Run your E2E tests to make sure nothing is broken, then come back."
- If docs not updated: "Run `/steez-document-release` to update CHANGELOG and docs."
...
Then tell the user: "Staging looks good. When you're ready for production, run `/steez-land-and-deploy` again."
**STOP.** The user can re-run `/steez-land-and-deploy` later for production.
...
```bash
$B snapshot -i -a -o ".steez/deploy-reports/post-deploy.png"
```
...
Save report to `.steez/deploy-reports/{date}-pr{number}-deploy.md`.
...
- If a production URL was verified: "Want extended monitoring? Run `/steez-canary <url>` to watch the site for the next 10 minutes."
- If performance data was collected: "Want a deeper performance analysis? Run `/steez-benchmark <url>`."
- "Need to update docs? Run `/steez-document-release` to sync README, CHANGELOG, and other docs with what you just shipped."
...
- **Single-pass verification, not continuous monitoring.** `/steez-land-and-deploy` checks once. `/steez-canary` does the extended monitoring loop.
